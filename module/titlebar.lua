...
titlebar_core.enable_tooltip = true
titlebar_core.fallback_name  = 'Client'
...
local create_vertical_bar = function(c, pos, bg, size)
	...
	titlebar_core(c, {position = pos, bg = bg, size = size}) : setup {
		{
			{
				titlebar_core.widget.closebutton(c),
				titlebar_core.widget.maximizedbutton(c),
				titlebar_core.widget.minimizebutton(c),
				spacing = dpi(7),
		...
		{
			{
				titlebar_core.widget.ontopbutton(c),
				titlebar_core.widget.floatingbutton(c),
				spacing = dpi(7),
				layout  = wibox.layout.fixed.vertical
			},
			margins = dpi(10),
